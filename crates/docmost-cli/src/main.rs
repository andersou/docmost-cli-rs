...
/// Link to a page in the web app, in the exact shape the client builds
/// it. The space slug comes with the page when `includeSpace` was requested
/// and is looked up otherwise.
async fn page_link(client: &DocmostClient, page: &Value) -> Result<String, AppError> {
    let slug_id = page["slugId"]
        .as_str()
        .ok_or_else(|| AppError::Failed("the page has no slugId".into()))?;
    let space_slug = match page["space"]["slug"].as_str() {
        Some(slug) => slug.to_owned(),
        None => {
            let space_id = page["spaceId"]
                .as_str()
                .ok_or_else(|| AppError::Failed("the page has no spaceId".into()))?;
            let space: Value = client.spaces().info(space_id).await?;
            space["slug"]
                .as_str()
                .ok_or_else(|| AppError::Failed("the space has no slug".into()))?
                .to_owned()
        }
    };
    Ok(links::page_url(
...
async fn execute_authenticated(
    ...
    match command {
        ...
        Command::Page(page) => match &page.action {
            ...
            PageAction::Url(args) => {
                let page: Value = client.pages().info(&args.id, None, true).await?;
                let url = page_link(client, &page).await?;
                emit(
                    output,
                    &json!({
                        "id": page["id"],
